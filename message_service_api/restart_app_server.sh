...
if [ $count -gt 0 ];then
    ...
    echo "当前启动地址： $1"
    nohup uwsgi --socket $1 --wsgi-file run.py --callable app --enable-threads --lazy-apps --evil-reload-on-as 1024 --evil-reload-on-rss 256 --listen 100 --processes 4 --threads 1 &
    if [ 0 == $? ];then
